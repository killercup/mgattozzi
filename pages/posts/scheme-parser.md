# Schemers - Parsers
<div class="subtitle">Published December th, 2016</div>
...
a number. It uses macros to build up it's parsers. We'll start small and
...
complete but it will give us an introduction to building a parser and
using it to place data into the appropriate data structures. We'll also
...
# Getting nom setup
We need another library! That means we need to open `Cargo.toml`!

It should look sort of like this right now:
...
between v2.0.0 and v2.1.0 (not including v2.1.0) use that. This way we get
bug fixes and the like but don't use a version that might break our code.
...
This is saying import the `nom` crate and we'll also be using the macros
that are exported from it. If you don't place `#[macro_use]` there then
we can't use them and then writing a parser with `nom` would be possible
but incredibly hard. Keep your `main.rs` file open still. We need to do
one last thing.
...
before `use` statements in a file but it really just comes down to
personal preference. It doesn't matter what the order is. With all of
...
Maybe the file got deleted, isn't there, or there was some kind of
...
bytes or [u8]. In fact it takes input as an immutable reference so
really it's &[u8].
...
the return! The other part is the type of data returned from being
...
between `:` and `>>` then store it into word. We can call this variable
...
`ws!()` it will stop once it hits a whitespace character.
...
case we're saying take whatever was stored in word turn it from `[u8]` to
a vector of bytes, `Vec<u8>` (`Vec` uses slices inside of it so the
promotion from `[]` to `Vec` is not unsafe), and then turn that into
a `String` and unwrap it even if it might fail. Now we'll make this
a little bit more robust later but in this case since we've been
...
and they'll run when you run `cargo test` as a command. It's amazing to
be able to do that. We're goint to write a test to make sure our string
...
things like "if" or "+" as a String but that's not type safe and harder
to make sure we match possibilities. So let's create a way to represent
...
more about them [here](https://doc.rust-lang.org/beta/book/traits.html) if you're interested
...
value! If you look user returns an `Op` here. What's really cool here is
...
than `User`. Well what's the `alt!()` doing? It's matching on alternate
...
put in it. In this case it's single characters but it works. What's this
...
action and maybe it'll make more sense.
...
Given specific inputs we're expecting certain outcomes. If you run the
...
that's not how we're going to represent things in the future. For now
...
`Op` and `args` to be a vector of `Strings`.
...
value. We'll now test both of them. One has arguments the other does not
...
2. `Procedure` has a field args that's just a vector of strings. This
   ...
   types following it. Create an enum `Value` that can handle `String`,
   `Op` or integers (make it hold an `i64` type). Modify `Procedure`'s
   `args` field to hold a `Vec<Value>` instead and modify the parser to do
   that.
...
All of this article's code is available at this [commit]().
